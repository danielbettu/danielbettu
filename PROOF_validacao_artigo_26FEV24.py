...
def calcular_media_ponderada(medias_atributo, cols, peso_col):
    medias_ponderadas = {}
    for col in cols:
        medias_ponderadas[col] = np.average(medias_atributo[col], weights=medias_atributo[peso_col])
    return medias_ponderadas

# Aplicação da função calcular_media_ponderada
pre_mean_litho_dom = calcular_media_ponderada(medias_atributo, cols, 'espessura')

def calcular_variancia_ponderada(medias_atributo, cols, peso_col, medias):
    variancias_ponderadas = {}
    for col in cols:
        diff = medias_atributo[col] - medias[col]
        variancias_ponderadas[col] = np.average(diff**2, weights=medias_atributo[peso_col])
    return variancias_ponderadas

# Aplicação da função calcular_variancia_ponderada
pre_var_litho_dom = calcular_variancia_ponderada(medias_atributo, cols, 'espessura', pre_mean_litho_dom)
...
# Aplicação da função calcular_media_ponderada
medias_atributo = merged_atrib_terr_prop.copy()
pre_mean_terr_prop = calcular_media_ponderada(medias_atributo, cols, 'espessura')
...
atrib_mean_thick = pd.DataFrame(mean_values).T
